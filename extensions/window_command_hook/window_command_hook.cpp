...
dllx double window_command_hook_raw(void* _in_ptr, double _in_ptr_size, double _arg_command) {
	...
	return window_command_hook(_arg_hwnd, (int)_arg_command);
}

extern bool window_command_unhook(GAME_HWND hwnd, int command);
dllx double window_command_unhook_raw(void* _in_ptr, double _in_ptr_size, double _arg_command) {
	auto _gmkb = gmk_buffer_args.data();
	if (_gmkb) _in_ptr = _gmkb;
	gml_istream _in(_in_ptr);
	GAME_HWND _arg_hwnd = (GAME_HWND)_in.read<uint64_t>();
	return window_command_unhook(_arg_hwnd, (int)_arg_command);
}

extern bool window_command_check(int command);
dllx double window_command_check_raw(double _arg_command) {
	// no buffer!
	return window_command_check((int)_arg_command);
}

extern int window_command_run(GAME_HWND hwnd, int wParam, int lParam);
dllx double window_command_run_raw(void* _in_ptr, double _in_ptr_size, double _arg_wParam, double _arg_lParam) {
	auto _gmkb = gmk_buffer_args.data();
	if (_gmkb) _in_ptr = _gmkb;
	gml_istream _in(_in_ptr);
	GAME_HWND _arg_hwnd = (GAME_HWND)_in.read<uint64_t>();
	return window_command_run(_arg_hwnd, (int)_arg_wParam, (int)_arg_lParam);
}

extern int window_command_get_active(GAME_HWND hwnd, int command);
dllx double window_command_get_active_raw(void* _in_ptr, double _in_ptr_size, double _arg_command) {
	auto _gmkb = gmk_buffer_args.data();
	if (_gmkb) _in_ptr = _gmkb;
	gml_istream _in(_in_ptr);
	GAME_HWND _arg_hwnd = (GAME_HWND)_in.read<uint64_t>();
	return window_command_get_active(_arg_hwnd, (int)_arg_command);
}

extern int window_command_set_active(GAME_HWND hwnd, int command, bool value);
dllx double window_command_set_active_raw(void* _in_ptr, double _in_ptr_size, double _arg_command, double _arg_value) {
	auto _gmkb = gmk_buffer_args.data();
	if (_gmkb) _in_ptr = _gmkb;
	gml_istream _in(_in_ptr);
	GAME_HWND _arg_hwnd = (GAME_HWND)_in.read<uint64_t>();
	return window_command_set_active(_arg_hwnd, (int)_arg_command, (bool)_arg_value);
}

extern bool window_get_background_redraw();
dllx double window_get_background_redraw_raw() {
	// no buffer!
	return window_get_background_redraw();
}

extern bool window_set_background_redraw(GAME_HWND hwnd, bool enable);
dllx double window_set_background_redraw_raw(void* _in_ptr, double _in_ptr_size, double _arg_enable) {
	auto _gmkb = gmk_buffer_args.data();
	if (_gmkb) _in_ptr = _gmkb;
	gml_istream _in(_in_ptr);
	GAME_HWND _arg_hwnd = (GAME_HWND)_in.read<uint64_t>();
	return window_set_background_redraw(_arg_hwnd, (bool)_arg_enable);
}
...
dllx double window_set_topmost_raw(void* _in_ptr, double _in_ptr_size, double _arg_enable) {
	auto _gmkb = gmk_buffer_args.data();
	if (_gmkb) _in_ptr = _gmkb;
	gml_istream _in(_in_ptr);
	GAME_HWND _arg_hwnd = (GAME_HWND)_in.read<uint64_t>();
	return window_set_topmost(_arg_hwnd, (bool)_arg_enable);
}
...
dllx double window_set_taskbar_button_visible_raw(void* _in_ptr, double _in_ptr_size, double _arg_show_button) {
	auto _gmkb = gmk_buffer_args.data();
	if (_gmkb) _in_ptr = _gmkb;
	gml_istream _in(_in_ptr);
	GAME_HWND _arg_hwnd = (GAME_HWND)_in.read<uint64_t>();
	return window_set_taskbar_button_visible(_arg_hwnd, (bool)_arg_show_button);
}
...
dllx double window_set_clickthrough_raw(void* _in_ptr, double _in_ptr_size, double _arg_enable_clickthrough) {
	auto _gmkb = gmk_buffer_args.data();
	if (_gmkb) _in_ptr = _gmkb;
	gml_istream _in(_in_ptr);
	GAME_HWND _arg_hwnd = (GAME_HWND)_in.read<uint64_t>();
	return window_set_clickthrough(_arg_hwnd, (bool)_arg_enable_clickthrough);
}
...
dllx double window_set_noactivate_raw(void* _in_ptr, double _in_ptr_size, double _arg_disable_activation) {
	auto _gmkb = gmk_buffer_args.data();
	if (_gmkb) _in_ptr = _gmkb;
	gml_istream _in(_in_ptr);
	GAME_HWND _arg_hwnd = (GAME_HWND)_in.read<uint64_t>();
	return window_set_noactivate(_arg_hwnd, (bool)_arg_disable_activation);
}

extern bool window_set_visible_w(GAME_HWND hwnd, bool visible);
dllx double window_set_visible_w_raw(void* _in_ptr, double _in_ptr_size, double _arg_visible) {
	auto _gmkb = gmk_buffer_args.data();
	if (_gmkb) _in_ptr = _gmkb;
	gml_istream _in(_in_ptr);
	GAME_HWND _arg_hwnd = (GAME_HWND)_in.read<uint64_t>();
	return window_set_visible_w(_arg_hwnd, (bool)_arg_visible);
}
...
dllg bool window_get_clickthrough(GAME_HWND hwnd) {
    auto exStyle = GetWindowLong(hwnd, GWL_EXSTYLE);
    return (exStyle & MY_GWL_CLICKTHROUGH) == MY_GWL_CLICKTHROUGH;
}
dllg bool window_set_clickthrough(GAME_HWND hwnd, bool enable_clickthrough) {
    auto exStyle = GetWindowLong(hwnd, GWL_EXSTYLE);
    if (!enable_clickthrough && (exStyle & WS_EX_LAYERED) != 0) {
        COLORREF crKey{};
        DWORD flags = 0;
        if (GetLayeredWindowAttributes(hwnd, &crKey, NULL, &flags) && (flags & (LWA_COLORKEY | LWA_ALPHA)) != 0) {
            // special case: if the window is chromakeyed, we do not want to disable WS_EX_LAYERED
